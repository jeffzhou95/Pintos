...
process_execute (const char *file_name) 
{
  ...
  // don't ruin prime file_name, so create a new file_name_copy.
...
setup_stack (void **esp, char *file_name) 
{
  ...
  // deal with argc
  for (cmd = strtok_r (copy, " ", &saveptr); cmd != NULL;
    cmd = strtok_r (NULL, " ", &saveptr))
    argc++;

  // deal with argv and esp
  ...
  while((int)*esp % 4 != 0) {
    *esp = *esp - sizeof(char);
    char char_temp = 0;
    memcpy(*esp, &char_temp, sizeof(char));
  }
  ...
  *esp = *esp - sizeof(int);
  memcpy(*esp, &ptr_temp, sizeof(int));
